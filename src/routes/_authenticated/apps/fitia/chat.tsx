...
function ChatPage() {
  ...
  return (
    <AppShell appSlug="fitia">
      <ScreenHeader title="Coach IA" subtitle="Tire dúvidas sobre execução, substituições e progressão." />

      <div className="space-y-3 mb-4">
        {msgs.length === 0 && !loading && (
          <div className="surface p-5 text-sm" style={{ color: "var(--n-500)" }}>
            Ex.: "Como fazer agachamento sem doer o joelho?" ou "Substitua supino reto por algo em casa."
          </div>
        )}
        {msgs.map((m, i) => (
