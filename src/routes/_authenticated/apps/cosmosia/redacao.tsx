      ...
      {result && (
        <div className="fade-up space-y-6">
          ...
          <div className="surface flex items-center justify-between rounded-2xl border border-indigo-500/30 bg-gradient-to-r from-indigo-950/40 to-neutral-900 p-6">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wider text-indigo-400">Nota Estimada ENEM</div>
              <h2 className="mt-1 text-4xl font-extrabold text-white sm:text-5xl">{result.totalScore} / 1000</h2>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {Object.entries(result.competencies).map(([comp, val], idx) => (
              <div key={comp} className="surface rounded-xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-indigo-400">Competência {idx + 1}</span>
                  <span className="rounded-full bg-indigo-500/20 px-2.5 py-1 text-xs font-extrabold text-indigo-300">
                    {val.score} / 200 pts
                  </span>
                </div>
                <p className="mt-2 text-xs text-white/70 leading-relaxed">{val.feedback}</p>
              </div>
            ))}
          </div>
          ...
          <div className="surface rounded-xl border border-white/10 bg-white/5 p-5">
            <div className="mb-2 text-xs font-bold uppercase tracking-wider text-indigo-400">Diagnóstico Geral</div>
            <p className="text-sm text-white/90 leading-relaxed">{result.generalFeedback}</p>
          </div>

          {/* Strengths & Improvements */}
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="surface rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-5">
            ...
            <div className="surface rounded-xl border border-amber-500/20 bg-amber-950/10 p-5">
              <div className="mb-3 flex items-center gap-2 font-semibold text-amber-400">
