        ...
        <div key={step.id} className="surface p-4 flex flex-col gap-2 relative overflow-hidden">
          <div className="flex items-center justify-between mb-1">
            <span className={`text-[10px] uppercase tracking-widest font-bold ${
              ...
              step.status === 'in-progress' ? 'text-amber-500' : 'text-muted-foreground'
            }`}>
              {step.status === 'completed' ? 'Concluído' : step.status === 'in-progress' ? 'Em curso' : 'Pendente'}
            </span>
          ...
          <div className="space-y-1">
            {step.tasks.map((task, i) => (
              <div key={i} className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                <div className={`h-1 w-1 rounded-full ${step.status === 'completed' ? 'bg-primary/50' : 'bg-muted-foreground/30'}`} />
                {task}
              </div>
            ))}
          </div>
